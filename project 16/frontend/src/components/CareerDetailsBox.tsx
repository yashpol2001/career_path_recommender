// CareerDetailsBox.tsx
import React from 'react';
import { ExternalLink, BookOpenCheck, Award, Rocket, FileCode2, Info } from 'lucide-react';
    ...
    <div className="bg-gradient-to-br from-white to-indigo-50 border border-indigo-100 rounded-3xl shadow-2xl px-10 py-10 mt-10 space-y-10 animate-fadeIn font-[Inter]">
      ...
      <div>
        ...
        <p className="text-gray-700 text-lg leading-relaxed tracking-wide font-medium">
          Imagine yourself as a future <strong>{data.career}</strong>. This roadmap was curated to help you learn smart, stand out, and shine confidently in your space.
        </p>
      </div>
      ...
      <div className="text-center pt-4">
        <a
          href={`https://www.google.com/search?q=${encodeURIComponent(data.career)}+role+skills`}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-semibold shadow-md hover:scale-105 transition"
        >
          <Info size={18} /> Learn More About {data.career}
        </a>
      </div>
    </div>
  );
